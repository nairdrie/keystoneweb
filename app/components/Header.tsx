...
import KeystoneLogoLight from './KeystoneLogoLight';
    ...
    <header className="fixed top-0 w-full z-50 bg-white/95 backdrop-blur-sm border-b border-slate-200">
      <nav className="mx-auto max-w-6xl px-4 sm:px-6 lg:px-8 py-4 flex items-center justify-between">
        ...
        <div className="hidden md:flex items-center gap-8">
          ...
          <Link
            href="/templates"
            className="px-6 py-2.5 rounded-full bg-blue-600 hover:bg-blue-700 text-white text-sm font-bold transition-colors shadow-md hover:shadow-lg"
          >
            Get Started
          </Link>
        </div>
      ...
      {isOpen && (
        <div className="md:hidden border-t border-slate-200 bg-white">
          <div className="px-4 py-4 space-y-3">
            ...
            <Link
              href="/templates"
              className="block w-full px-6 py-2.5 rounded-full bg-blue-600 hover:bg-blue-700 text-white text-sm font-bold text-center transition-colors shadow-md"
              onClick={() => setIsOpen(false)}
            >
              Get Started
            </Link>
