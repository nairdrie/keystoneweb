...
export default function SEOPanel({ siteId }: SEOPanelProps) {
  ...
  const [seoMeta, setSeoMeta] = useState<SeoMeta>({ seoTitle: '', seoDescription: '' });
  ...
  const handleSave = async () => {
    ...
    try {
      const res = await fetch('/api/seo/business-profile', {
        ...
        body: JSON.stringify({ siteId, businessProfile: profile, socialLinks, seoTitle: seoMeta.seoTitle, seoDescription: seoMeta.seoDescription }),
      });
      if (res.ok) {
  ...
  return (
    <div className="p-4 space-y-4">
      ...
      <div className="space-y-3">
        <div className="text-[10px] font-bold uppercase text-slate-400 tracking-widest border-b border-slate-100 pb-1">
          Search Engine Listing
        </div>
        <div>
          <label className="block text-[10px] font-bold uppercase text-slate-500 tracking-wide mb-1">SEO Title</label>
          <input
            type="text"
            value={seoMeta.seoTitle}
            onChange={(e) => setSeoMeta(prev => ({ ...prev, seoTitle: e.target.value }))}
        ...
        <div>
          ...
          <textarea
            value={seoMeta.seoDescription}
            onChange={(e) => setSeoMeta(prev => ({ ...prev, seoDescription: e.target.value }))}
          ...
          />
          <p className="text-[10px] text-slate-400 mt-1">Shown under your title in search results. Keep under 160 characters.</p>
        </div>
        <button
          onClick={handleSave}
          disabled={saving || !siteId}
          className="w-full py-2 text-white font-bold text-sm rounded-lg transition-all hover:brightness-110 disabled:opacity-50"
