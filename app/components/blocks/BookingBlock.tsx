    ...
    return (
        <div className="space-y-4">
            ...
            {services.map(service => (
                <div key={service.id} className={`flex items-start gap-3 p-3 rounded-lg border ${service.is_active ? 'border-slate-200 bg-white' : 'border-slate-100 bg-slate-50 opacity-60'}`}>
        ...
        <div className="space-y-4">
            <div>
                ...
                <select
                    ...
                    onChange={e => setLocal({ ...local, buffer_minutes: parseInt(e.target.value) })}
            ...
            <div>
                ...
                <div className="space-y-2">
                    <label className="flex items-center gap-2 cursor-pointer">
                        <input
                            type="checkbox"
                            checked={local.payment_methods?.none !== false}
                            onChange={() => setLocal({
                                ...local,
                                payment_methods: { ...local.payment_methods, none: !local.payment_methods?.none }
                            })}
                            className="rounded accent-blue-600"
                        />
                    ...
                    <label className="flex items-center gap-2 cursor-pointer">
                        <input
                            type="checkbox"
                            checked={local.payment_methods?.etransfer || false}
                            onChange={() => setLocal({
                                ...local,
                                payment_methods: { ...local.payment_methods, etransfer: !local.payment_methods?.etransfer }
                            })}
                            className="rounded accent-blue-600"
                        />
...
function BookingFlow({ siteId, palette }: { siteId: string; palette: Record<string, string> }) {
    const [step, setStep] = useState<'service' | 'date' | 'time' | 'form' | 'confirmation'>('service');
    const [services, setServices] = useState<Service[]>([]);
    ...
    const handleSubmit = async () => {
        ...
        const paymentMethod = settings?.payment_methods?.etransfer ? 'etransfer'
            : settings?.payment_methods?.none !== false ? 'none'
                : 'none';
